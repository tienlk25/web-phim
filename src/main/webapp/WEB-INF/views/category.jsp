...
<body
	...
	<div class="container">
		...
		<h2 class="list-product-title text-light">${name.name }</h2>
		<!--  		<div class="col-md-3 col-sm-6 col-12">
						<div class=" card-product mb-3">
							<a href="${pageContext.request.contextPath}/details/${post.id}">
								<img class="card-img-top image "
								src="${pageContext.request.contextPath}/files/${post.images.get(0).name }"
								width="" alt="Card image cap">
							</a>
							<div class="card-title"
								style="height: 55px; background-color: #0a0a0a;">
								<span class="card-title product-title text-center"> <a
									href="${pageContext.request.contextPath}/details/${post.id}"
									class="btn-detail" data-toggle="tooltip"
									title="${post.description }" style="color: #cdcdcd;"> <c:out
											value="${post.ten}"></c:out>: <c:out value="${post.title}"></c:out>
								</a>
								</span>
							</div>
						</div>
